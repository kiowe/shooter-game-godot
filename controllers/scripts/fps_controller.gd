...
@export var SPEED_SPRINTING : float = 7.5
...
func _input(event):
	if event.is_action_pressed("exit"):
		get_tree().quit()
	if event.is_action_pressed("crouch") and is_on_floor() and TOGGLE_CROUCH == true: # Press to crouch and uncrouch
		toggle_crouch()
	if event.is_action_pressed("crouch") and (_is_crouching == false) and is_on_floor() and TOGGLE_CROUCH == false: # Hold to crouch
		crouching(true)
	if event.is_action_released("crouch") and (_is_crouching == true) and TOGGLE_CROUCH == false: # Release to uncrouch
...
func _physics_process(delta: float) -> void:
	...
	if direction:
		...
		velocity.x = vel.normalized().x * move_toward(Vector2(velocity.x, velocity.z).length(), 0, DECELERATION)
		velocity.z = vel.normalized().y * move_toward(Vector2(velocity.x, velocity.z).length(), 0, DECELERATION)
